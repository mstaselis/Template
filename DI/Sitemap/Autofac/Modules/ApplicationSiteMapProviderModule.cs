...
{
    public class ApplicationSiteMapProviderModule
        : global::Autofac.Module
    {
        protected override void Load(ContainerBuilder builder)
        {
            ...
            string absoluteAdminFileName = HostingEnvironment.MapPath("~/Areas/Admin/Mvc.sitemap");
            ...
            string[] includeAssembliesForScan = new string[] { "Web" };
...
// typeof(SiteMapNodeVisibilityProviderStrategy)
            ...
            builder.RegisterType<ReservedAttributeNameProvider>()
                ...
                .WithParameter("attributesToIgnore", new string[]{"String", "icon"});
            ...
            builder.RegisterType<RuntimeFileCacheDependency>()
               .Named<ICacheDependency>("cacheDependency2")
               .WithParameter("fileName", absoluteAdminFileName);

            builder.RegisterType<CacheDetails>()
                .Named<ICacheDetails>("cacheDetails2")
                .WithParameter("absoluteCacheExpiration", absoluteCacheExpiration)
                .WithParameter("slidingCacheExpiration", TimeSpan.MinValue)
                .WithParameter(
                    (p, c) => p.Name == "cacheDependency",
                    (p, c) => c.ResolveNamed<ICacheDependency>("cacheDependency2"));



            // Prepare for our node providers
            builder.RegisterType<FileXmlSource>()
                .Named<IXmlSource>("xmlSource2")
                .WithParameter("fileName", absoluteAdminFileName);
            ...
            // Register the sitemap node providers
            builder.RegisterType<XmlSiteMapNodeProvider>()
                .Named<ISiteMapNodeProvider>("xmlSiteMapNodeProvider2")
                .WithParameter("includeRootNode", true)
                .WithParameter("useNestedDynamicNodeRecursion", false)
                .WithParameter(
                    (p, c) => p.Name == "xmlSource",
                    (p, c) => c.ResolveNamed<IXmlSource>("xmlSource2"));

            builder.RegisterType<ReflectionSiteMapNodeProvider>()
                .Named<ISiteMapNodeProvider>("reflectionSiteMapNodeProvider2")
                .WithParameter("includeAssemblies", includeAssembliesForScan)
                .WithParameter("excludeAssemblies", new string[0]);

            builder.RegisterType<CompositeSiteMapNodeProvider>()
                .Named<ISiteMapNodeProvider>("siteMapNodeProvider2")
                .WithParameter(
                    (p, c) => p.Name == "siteMapNodeProviders",
                    (p, c) => new[]
                        {
                            c.ResolveNamed<ISiteMapNodeProvider>("xmlSiteMapNodeProvider2"),
                            c.ResolveNamed<ISiteMapNodeProvider>("reflectionSiteMapNodeProvider2")
                        });

            // Register the sitemap builders
            builder.RegisterType<SiteMapBuilder>()
                .Named<ISiteMapBuilder>("siteMapBuilder2")
                .WithParameter(
                    (p, c) => p.Name == "siteMapNodeProvider",
                    (p, c) => c.ResolveNamed<ISiteMapNodeProvider>("siteMapNodeProvider2"));

            // Configure the builder sets
            builder.RegisterType<SiteMapBuilderSet>()
                   .Named<ISiteMapBuilderSet>("builderSet2")
                   .WithParameter("instanceName", "AdminMenu")
                   .WithParameter("securityTrimmingEnabled", securityTrimmingEnabled)
                   .WithParameter("enableLocalization", enableLocalization)
                   .WithParameter("visibilityAffectsDescendants", visibilityAffectsDescendants)
                   .WithParameter("useTitleIfDescriptionNotProvided", useTitleIfDescriptionNotProvided)
                   .WithParameter(
                        (p, c) => p.Name == "siteMapBuilder",
                        (p, c) => c.ResolveNamed<ISiteMapBuilder>("siteMapBuilder2"))
                   .WithParameter(
                        (p, c) => p.Name == "cacheDetails",
                        (p, c) => c.ResolveNamed<ICacheDetails>("cacheDetails2"));
            ...
            builder.RegisterType<SiteMapBuilderSetStrategy>()
                ...
                .WithParameter(
                    ...
                    (p, c) => new[]
                        {
                          c.ResolveNamed<ISiteMapBuilderSet>("builderSet2"),
                          c.ResolveNamed<ISiteMapBuilderSet>("builderSet1")
                        });

            //register localization
            string resourceBaseName = "Framework.Resources.Menu";

            // A reference to the assembly where your resources reside.
            Assembly resourceAssembly = typeof(Framework.Resources.Menu).Assembly;
            ...
            builder.Register(c => new ResourceManager(resourceBaseName, resourceAssembly)).As<ResourceManager>();           

            // Register the ResourceManagerStringLocalizer (uses the ResourceManger)
            builder.RegisterType<ResourceManagerStringLocalizer>().As<IStringLocalizer>();

            //register cache map
            builder.RegisterType<CustomSiteMapCacheKeyToBuilderSetMapper>().As<ISiteMapCacheKeyToBuilderSetMapper>();   
        }
    }
}
